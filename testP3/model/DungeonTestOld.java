...
public class DungeonTestOld {
  ...
  public void movingThePlayerNoWrap() {
    while (dungeonNoWrap.getPlayerLocation().getX() != 0) {
      ...
      if (loc.equals(dungeonNoWrap.getEnd())) {
        dungeonNoWrap.shoot(1, model.enums.Direction.WEST);
        dungeonNoWrap.shoot(1, model.enums.Direction.WEST);
      }
      dungeonNoWrap.movePlayer(model.enums.Direction.WEST);
    }
    while (dungeonNoWrap.getPlayerLocation().getY() != 0) {
      ...
      if (loc.equals(dungeonNoWrap.getEnd())) {
        dungeonNoWrap.shoot(1, model.enums.Direction.NORTH);
        dungeonNoWrap.shoot(1, model.enums.Direction.NORTH);
      }
      dungeonNoWrap.movePlayer(model.enums.Direction.NORTH);
    }
  ...
  public void movingThePlayerWrap() {
    while (dungeonWrap.getPlayerLocation().getX() != 0) {
      ...
      if (loc.equals(dungeonWrap.getEnd())) {
        dungeonWrap.shoot(1, model.enums.Direction.WEST);
        dungeonWrap.shoot(1, model.enums.Direction.WEST);
      }
      dungeonWrap.movePlayer(model.enums.Direction.WEST);
    }
    while (dungeonWrap.getPlayerLocation().getY() != 0) {
      ...
      if (loc.equals(dungeonWrap.getEnd())) {
        dungeonWrap.shoot(1, model.enums.Direction.NORTH);
        dungeonWrap.shoot(1, model.enums.Direction.NORTH);
      }
      dungeonWrap.movePlayer(model.enums.Direction.NORTH);
    }
    ...
    dungeonWrap.movePlayer(model.enums.Direction.EAST);
    assertEquals(new Location(0, 0), dungeonWrap.getPlayerLocation());
    dungeonWrap.movePlayer(model.enums.Direction.NORTH);
    ...
    dungeonWrap.movePlayer(model.enums.Direction.SOUTH);
    assertEquals(new Location(0, 0), dungeonWrap.getPlayerLocation());
  }
  ...
  public void treasureHuntNoWrap() {
    ...
    Location loc = new Location(dungeonNoWrap.getPlayerLocation().getX() + 1,
            dungeonNoWrap.getPlayerLocation().getY());
    if (loc.equals(dungeonNoWrap.getEnd())) {
      dungeonNoWrap.shoot(1, model.enums.Direction.EAST);
      dungeonNoWrap.shoot(1, model.enums.Direction.EAST);
    }
    dungeonNoWrap.movePlayer(model.enums.Direction.EAST);
    ...
    loc = new Location(dungeonNoWrap.getPlayerLocation().getX(),
            dungeonNoWrap.getPlayerLocation().getY() + 1);
    if (loc.equals(dungeonNoWrap.getEnd())) {
      dungeonNoWrap.shoot(1, model.enums.Direction.SOUTH);
      dungeonNoWrap.shoot(1, model.enums.Direction.SOUTH);
    }
    dungeonNoWrap.movePlayer(model.enums.Direction.SOUTH);
  ...
  public void treasureHuntWrap() {
    ...
    if (loc.equals(dungeonWrap.getEnd())) {
      dungeonWrap.shoot(1, model.enums.Direction.EAST);
      dungeonWrap.shoot(1, model.enums.Direction.EAST);
    }
    dungeonWrap.movePlayer(model.enums.Direction.EAST);
  ...
  public void correctTreasurePercent1() {
    ...
    while (dung.getPlayerLocation().getX() != 0) {
      ...
      if (loc.equals(dung.getEnd())) {
        dung.shoot(1, model.enums.Direction.WEST);
        dung.shoot(1, model.enums.Direction.WEST);
      }
      dung.movePlayer(model.enums.Direction.WEST);
    }
    while (dung.getPlayerLocation().getY() != 0) {
      Location loc = new Location(dung.getPlayerLocation().getX(),
              dung.getPlayerLocation().getY() - 1);
      if (loc.equals(dung.getEnd())) {
        dung.shoot(1, model.enums.Direction.NORTH);
        dung.shoot(1, model.enums.Direction.NORTH);
      }
      dung.movePlayer(model.enums.Direction.NORTH);
    }
    int count = 0;
    ...
    for (int y = 0; y < dung.getHeight(); y++) {
        ...
        if (moving_east) {
          ...
          if (dung.getPlayerLocation().getX() + 1 < dung.getWidth()) {
            if (dung.getCave(loc).getMonster() != null
              ...
              dung.shoot(1, model.enums.Direction.EAST);
              dung.shoot(1, model.enums.Direction.EAST);
            }
          }

          dung.movePlayer(model.enums.Direction.EAST);
        } else {
      ...
      if (dung.getPlayerLocation().getY() + 1 < dung.getHeight()) {
        if (dung.getCave(loc).getMonster() != null && !dung.getCave(loc).getMonster().isDead()) {
          dung.shoot(1, model.enums.Direction.SOUTH);
          dung.shoot(1, model.enums.Direction.SOUTH);
        }
      }
      dung.movePlayer(model.enums.Direction.SOUTH);
  ...
  public void solvedIt() {
    ...
    assertTrue(dung.movePlayer(model.enums.Direction.EAST));
    assertTrue(dung.movePlayer(model.enums.Direction.SOUTH));
  ...
  public void getCaveTest() {
    ...
    assertNotEquals(check.getItems().get(model.enums.CaveObject.DIAMOND),
            myCave.getItems().get(model.enums.CaveObject.DIAMOND));
    assertNotEquals(check.getItems().get(model.enums.CaveObject.RUBY),
            myCave.getItems().get(model.enums.CaveObject.RUBY));
    assertNotEquals(check.getItems().get(model.enums.CaveObject.SAPPHIRE),
            myCave.getItems().get(model.enums.CaveObject.SAPPHIRE));
  }
  ...
  public void arrowTest1() {
    ...
    for (int y = 0; y < dungeonWrap.getHeight(); y++) {
      for (int x = 0; x < dungeonWrap.getWidth(); x++) {
        ...
        if (cave.getItems().get(model.enums.CaveObject.CROOKEDARROW) > 0) {
          count++;
        }
      }
    }
  ...
  public void smellTest() {
    ...
    for (Cave cave : first) {
      assertEquals(model.enums.Smell.PUNGENT, cave.getSmell());
      for (Location loc : cave.getDirections().values()) {
        if (!loc.equals(end.getLocation())) {
          assertEquals(model.enums.Smell.LIGHT, dungeonWrap.getCave(loc).getSmell());
        }
      }
    }
  }
      ...
      for (int y = 0; y < dung.getHeight(); y++) {
        for (int x = 0; x < dung.getWidth(); x++) {
          ...
          if (!gotSmell) {
            ...
            if (count == 0) {
              assertEquals(model.enums.Smell.NONE, cave.getSmell());
            } else if (count == 1) {
              assertEquals(model.enums.Smell.LIGHT, cave.getSmell());
            } else {
              assertEquals(model.enums.Smell.PUNGENT, cave.getSmell());
            }
          }
  ...
  public void shoot1() {
    ...
    dung.movePlayer(model.enums.Direction.EAST);
    dung.movePlayer(model.enums.Direction.EAST);
    assertTrue(dung.shoot(1, model.enums.Direction.EAST));
    assertTrue(dung.shoot(1, model.enums.Direction.EAST));
    dung.movePlayer(model.enums.Direction.EAST);
  ...
  public void shoot2() {
    Dungeon dung = dungeon(6, 6, 25, false, 0,
            "jack", false, 1);
    assertFalse(dung.shoot(1, model.enums.Direction.EAST));
    assertFalse(dung.shoot(1, model.enums.Direction.EAST));
    dung.movePlayer(model.enums.Direction.EAST);
    dung.search();
  ...
  public void shoot3() {
    ...
    dung.movePlayer(model.enums.Direction.EAST);
    dung.movePlayer(model.enums.Direction.EAST);
    assertFalse(dung.shoot(2, model.enums.Direction.EAST));
    assertFalse(dung.shoot(2, model.enums.Direction.EAST));
    dung.movePlayer(model.enums.Direction.EAST);
    assertTrue(dung.hasLost());
    assertFalse(dung.hasSolved());
  ...
  public void shoot4() {
    ...
    dung.movePlayer(model.enums.Direction.SOUTH);
    dung.movePlayer(model.enums.Direction.SOUTH);
    assertTrue(dung.shoot(3, model.enums.Direction.WEST));
    assertTrue(dung.shoot(3, model.enums.Direction.WEST));
    dung.movePlayer(model.enums.Direction.EAST);
    dung.movePlayer(model.enums.Direction.EAST);
    dung.movePlayer(model.enums.Direction.EAST);
  ...
  public void shoot5() {
    ...
    dung.movePlayer(model.enums.Direction.SOUTH);
    dung.movePlayer(model.enums.Direction.SOUTH);
    assertTrue(dung.shoot(5, model.enums.Direction.WEST));
    assertTrue(dung.shoot(5, model.enums.Direction.WEST));
    dung.movePlayer(model.enums.Direction.EAST);
    dung.movePlayer(model.enums.Direction.EAST);
    dung.movePlayer(model.enums.Direction.EAST);
  ...
  public void shoot6() {
    ...
    dung.movePlayer(model.enums.Direction.EAST);
    dung.movePlayer(model.enums.Direction.EAST);
    assertTrue(dung.shoot(5, model.enums.Direction.NORTH));
    assertTrue(dung.shoot(5, model.enums.Direction.NORTH));
    dung.movePlayer(model.enums.Direction.NORTH);
    dung.movePlayer(model.enums.Direction.NORTH);
    dung.movePlayer(model.enums.Direction.NORTH);
  ...
  public void shootAround() {
    ...
    assertTrue(dung.shoot(2, model.enums.Direction.NORTH));
    ...
    assertTrue(dung.shoot(3, model.enums.Direction.NORTH));
    ...
    assertTrue(dung.shoot(4, model.enums.Direction.NORTH));
